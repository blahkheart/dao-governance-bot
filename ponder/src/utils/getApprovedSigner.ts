...
import { SignedKeyRequestMetadataABI, keyGatewayAbi } from "../abi";
import { logger } from "./logger";

const { FARCASTER_BOT_MNEMONIC } = process.env;

if (!FARCASTER_BOT_MNEMONIC) {
  throw new Error("FARCASTER_BOT_MNEMONIC is not set");
}
...
const appendSignerUuidAndUsernameToEnv = (signer_uuid: string) => {
  ...
  fs.readFile(envPath, "utf8", (err, data) => {
    ...
    fs.writeFile(envPath, newContent, "utf8", (err) => {
      ...
      console.log(
        "SIGNER_UUID appended to .env file.\nPlease run `npm run ponder:dev` to continue.\n"
      );
    });
  });
};
